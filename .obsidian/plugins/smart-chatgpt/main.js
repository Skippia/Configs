...
function create_actions_proxy(ctx, actions_source) {
  ...
  const is_plain_object4 = (val) => typeof val === "object" && val !== null && !Array.isArray(val);
  ...
  const is_action_object = (val) => is_plain_object4(val) && is_function(val.action);
  ...
  const should_bucket_actions = (val) => {
    if (!is_plain_object4(val)) return false;
  ...
  const clone_descriptor = (descriptor) => {
    ...
    const cloned = is_plain_object4(descriptor.value) ? clone_with_descriptors(descriptor.value) : descriptor.value;
  ...
  const scope_actions_for = () => {
    ...
    return bucket && is_plain_object4(bucket) ? bucket : null;
  };
...
// src/views/smart_chat_codeblock.js
var import_obsidian44 = require("obsidian");

// src/utils/dropdown_label.js
var HOST_PLATFORM_LABELS = {
  "chatgpt.com": "ChatGPT",
...
var shorten_id_segment = (segment) => {
...
var platform_label_from_url = (url, fallback = "Link") => {
  try {
...
var format_dropdown_label = (url, platform_label = null) => {
...
// src/utils/smart_chat_codeblock.helpers.js
var import_url = require("url");
function extract_links_from_source({ codeblock_source, link_regex }) {
...
function normalize_url_value(url) {
  try {
    const normalized_url = new import_url.URL(url);
...
function resolve_initial_fallback_url({ initial_fallback_url, fallback_url }) {
  if (initial_fallback_url && typeof initial_fallback_url === "string") {
    return initial_fallback_url;
  }
  return fallback_url;
}
function resolve_initial_link_from_links({ links = [], initial_fallback_url, fallback_url }) {
...
function prefix_missing_chat_lines({ lines, start, end, link_regex, now_seconds }) {
  const regex = link_regex || /(https?:\/\/[^\s]+)/g;
  ...
  for (let index = start + 1; index < end; index++) {
    ...
    const found = line.match(regex) || [];
    if (found.length > 0) {
      const timestamp_in_seconds = typeof now_seconds === "number" ? now_seconds : Math.floor(Date.now() / 1e3);
...
function is_grok_thread_link(url) {
  const SUPPORTED_DOMAINS2 = [
    "grok.com",
    "www.grok.com"
  ];
  try {
    const u = new import_url.URL(url);
    if (!SUPPORTED_DOMAINS2.includes(u.hostname)) return false;
    const segments = (u.pathname || "").split("/").filter(Boolean).map((s) => String(s).toLowerCase());
...
function is_openwebui_thread_link(url) {
  try {
    const parsed_url = new import_url.URL(url);
    const segments = (parsed_url.pathname || "").split("/").filter(Boolean).map((segment) => String(segment).toLowerCase());
...
// src/views/smart_chat_codeblock.js
var DEFAULT_WEBVIEW_USERAGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.6723.191 Safari/537.36";
var DEFAULT_WEBVIEW_PREFERENCES = "nativeWindowOpen=yes, contextIsolation=yes";
var HELP_DOC_URL = "https://smartconnections.app/smart-chat/codeblock/";
var footer_button_labels = () => [
  "Refresh",
  "Build context",
  "Open in browser",
  "Copy link",
...
var MOBILE_HINT_TEXT = "Webview unavailable on mobile. Use Open + Copy.";
var SmartChatCodeblock = class {
  ...
  _build_help_button(parent_el) {
    ...
    try {
      (0, import_obsidian44.setIcon)(btn, "help-circle");
    } catch (_) {
      btn.textContent = "?";
    }
    btn.onclick = () => {
  ...
  _is_mobile_app() {
    try {
      if (import_obsidian44.Platform?.isMobileApp) return true;
    } catch (_) {
    }
  ...
  async _maybe_render_mark_active_ui(url) {
    ...
    if (!this.mark_done_button_el.dataset?.scMarkDoneLabel) {
      this.mark_done_button_el.dataset.scMarkDoneLabel = this.mark_done_button_el.textContent || (this._MARK_DONE_LABEL || "Mark done");
    }
  ...
  async _insert_link_into_codeblock(url) {
    ...
    if (this.ctx && this.ctx.replaceCode) {
      ...
      const updated_source2 = text.split("\n").slice(line_start + 1, line_end).join("\n");
      this.source = updated_source2;
      this.links = this._extract_links(this.source);
...
.cm-content.cm-lineWrapping:has(.sc-dynamic-codeblock)>.cm-embed-block:has(.sc-dynamic-codeblock){
  ...
  async _render_thread_state_ui(url) {
    ...
    if (is_http) {
      is_thread_link = typeof this._is_thread_link === "function" ? this._is_thread_link(url) : true;
    }
    let meta = null;
    if (is_thread_link) {
...
// src/utils/chatgpt_thread_link.js
...
var GPT_THREAD_REGEX = /^\/g\/[^/]+\/c\/[a-f0-9-]+\/?$/i;
var CODEX_TASK_REGEX = /^\/codex\/tasks\/[a-z0-9-_]+\/?$/i;
var CHAT_THREAD_REGEX = /^\/c\/[a-f0-9-]+\/?$/i;
var SORA_DRAFT_REGEX = /^\/d\/[a-z0-9-_]+\/?$/i;
var SORA_PUB_REGEX = /^\/p\/s_[a-f0-9]+\/?$/i;
var SORA_TASK_REGEX = /^\/t\/[a-f0-9-]+\/?$/i;
function is_chatgpt_thread_link(url) {
  try {
    const u = new import_url2.URL(url);
    if (!SUPPORTED_DOMAINS.includes(u.hostname)) return false;
    const path = u.pathname;
    return CHAT_THREAD_REGEX.test(path) || GPT_THREAD_REGEX.test(path) || CODEX_TASK_REGEX.test(path) || SORA_DRAFT_REGEX.test(path) || SORA_PUB_REGEX.test(path) || SORA_TASK_REGEX.test(path);
  } catch {
    return false;
  }
}

// src/views/smart_chatgpt_codeblock.js
...
// src/views/smart_claude_codeblock.js
var SmartClaudeCodeblock = class extends SmartChatCodeblock {
  ...
  constructor(opts = {}) {
    ...
    this._INITIAL_FALLBACK_URL = "https://claude.ai/chat";
    this.links = this._extract_links(this.source);
    const not_done_link_obj = this.links.find((obj) => !obj.done);
    this.initial_link = not_done_link_obj ? not_done_link_obj.url : this._INITIAL_FALLBACK_URL;
  ...
  async _mark_thread_done_in_codeblock(url) {
    ...
    this.webview_el?.setAttribute("src", this._INITIAL_FALLBACK_URL || "https://claude.ai/chat");
    this.current_url = this._INITIAL_FALLBACK_URL || "https://claude.ai/chat";
  }
  _find_next_undone_url(file_data, start, end, done_index) {
    if (done_index < 0) return null;
    const lines = file_data.split("\n");
...
// src/views/smart_gemini_codeblock.js
...
// src/views/smart_deepseek_codeblock.js
...
// src/views/smart_perplexity_codeblock.js
...
// src/views/smart_grok_codeblock.js
...
// src/views/smart_aistudio_codeblock.js
...
// src/views/smart_openwebui_codeblock.js
var DEFAULT_OPENWEBUI_BASE_URL = "http://localhost:3000/";
var normalize_openwebui_base_url = (value) => {
  const raw_value = String(value || "").trim();
  if (!raw_value) return DEFAULT_OPENWEBUI_BASE_URL;
  const try_parse = (candidate) => {
    try {
  ...
  return try_parse(raw_value) || try_parse(`http://${raw_value}`) || DEFAULT_OPENWEBUI_BASE_URL;
};
var SmartOpenWebuiCodeblock = class extends SmartChatCodeblock {
  /**
   * @param {Object} options
   * @param {import('obsidian').Plugin} options.plugin - The parent plugin instance.
   * @param {import('obsidian').TFile} options.file - The file containing the codeblock.
   ...
   * @param {number} options.line_end - The end line of the codeblock.
   * @param {HTMLElement} options.container_el - The container where this codeblock UI is rendered.
   * @param {string} options.source - The raw text inside the ```smart-openwebui codeblock.
   */
  constructor(opts = {}) {
    super(opts);
    this.platform_label = "Open WebUI";
    this.link_regex = /(https?:\/\/[^\s]+)/g;
    this.links = this._extract_links(this.source);
    this._FALLBACK_URL = normalize_openwebui_base_url(this.plugin?.settings?.openwebui_base_url);
    const not_done_link_obj = this.links.find((obj) => !obj.done);
    this.initial_link = not_done_link_obj ? not_done_link_obj.url : this._FALLBACK_URL;
    this.last_detected_url = this.initial_link;
    this.current_url = this.initial_link;
  ...
  _find_codeblock_boundaries(file_data) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      if (current_start === -1 && line.trim().startsWith("```smart-openwebui")) {
        current_start = i;
      } else if (current_start >= 0 && line.trim().startsWith("```")) {
        blocks.push({ start: current_start, end: i });
        current_start = -1;
  ...
  _is_thread_link(url) {
    return is_openwebui_thread_link(url);
  }
  async _render_save_ui(url) {
    this._set_status_text("");
    this._hide_mark_done_button();
    ...
    if (!this._is_thread_link(url)) {
      this._set_status_text("Not an Open WebUI chat link (no save/done).");
      return;
    }
    const link_to_check = this._normalize_url(url);
    const is_done = await this._check_if_done(link_to_check);
    if (!is_done) {
      ...
      this.mark_done_button_el.onclick = async () => {
        await this._mark_thread_done_in_codeblock(link_to_check);
        this.plugin.env?.events?.emit("chat_codeblock:marked_done", { url: link_to_check });
        this.plugin.notices.show("Marked chat as done.");
        this._render_save_ui(this.current_url);
      };
    }
  }
...
var SmartChatGPTView = class extends import_obsidian45.ItemView {
...
// node_modules/obsidian-smart-env/src/views/smart_plugin_settings_tab.js
var SmartPluginSettingsTab = class extends import_obsidian47.PluginSettingTab {
...
var SmartEnvSettingTab = class extends import_obsidian47.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
    this.header_container = null;
...
// src/views/settings_tab.js
var SmartChatgptSettingTab = class extends SmartPluginSettingsTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  async render_plugin_settings(container) {
    if (!container) return;
    container.empty?.();
    new import_obsidian48.Setting(container).setName("Height (px)").setDesc("Iframe height for embedded webviews.").addText((txt) => {
    ...
    const zoom_setting = new import_obsidian48.Setting(container).setName("Zoom").setDesc("Zoom factor for all webviews.");
    let slider_component = null;
    const current_value_el = zoom_setting.settingEl.createEl("div", {
      text: `Current: ${Number(this.plugin.settings.zoom_factor || 1).toFixed(1)}`
    });
    ...
    new import_obsidian48.Setting(container).setName("Open WebUI URL").setDesc("Base URL for your Open WebUI instance (used by the smart-openwebui codeblock). Example: http://localhost:3000").addText((txt) => {
      txt.setPlaceholder("http://localhost:3000").setValue(String(this.plugin.settings.openwebui_base_url || "")).onChange(async (value) => {
...
var SmartChatgptPlugin = class extends SmartPlugin {
  get env() {
...
`);
  ...
  register_chatgpt_view() {
    ...
    const method_name = SmartChatGPTView.view_type.replace("smart-", "").replace(/-/g, "_");
